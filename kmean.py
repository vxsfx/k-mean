...
def get_V(k_points, d_point):
    short_v = [k_points[0][x] -d_point[x] ,  k_points[0][y] - d_point[y]]
    short_d = math.sqrt(short_v[x]*short_v[x] + short_v[y]*short_v[y] )
    k = 0
    
    for kp_ind in range(1, k_points.__len__()):
        v = [k_points[kp_ind][x] - d_point[x], k_points[kp_ind][y]- d_point[y]]
...
def k_mean(data, iterations, k_val):
    ...
    for i in range(iterations):
        #k_means = [[0,0] for i in range(k_val)]
        # use totals instead of vect / 2 => for each point ???
        k_totals = [0 for i in range(k_val)]
        for vector in data:
            t_v, k_index  = get_V(k_points, vector)
            k_totals[k_index] += 1
            k_means[k_index][x] -= t_v[x]#(k_means[k_index][x] - t_v[x]) /2 
            k_means[k_index][y] -= t_v[y] #(k_means[k_index][y] - t_v[y] ) /2   #ffffffffffffffuuuuuuuuuuuuu y was x :(
            
            
            
            #k_means[k_index]
        for k_ind in range(k_val):
            k_means[k_ind][x] = k_means[k_ind][x] / k_totals[k_ind]
            k_means[k_ind][y] = k_means[k_ind][y] / k_totals[k_ind]    #ffffffffffffffuuuuuuuuuuuuu y was x :(
            


        for kp in range(k_val):
             k_points[kp] = [k_points[kp][x] + k_means[kp][x], k_points[kp][y] + k_means[kp][y]]
...
if __name__ == "__main__":
    ...
    d_tl = [[x, y] for x,y in zip(np.random.randint(10, 40, 50), np.random.randint(40, 50, 50))]
    d_br = [[x, y] for x,y in zip(np.random.randint(40, 50, 50), np.random.randint(0, 10, 50))]
